...
用**先進先出（FIFO）**：賣出時從最早買進的那批開始結轉成本（Kevin 指定，
也與美國券商預設一致，對帳單比較對得起來）。股息與沒有股數異動的現金流
直接計入已實現。見 `app.js` 的 `splitCost()`，內部維護一個先進先出的批次佇列。
...
**不變條件：`已實現 + 未實現` 必然等於 `累計損益`。** 改動這段之後一定要驗這個
—— 逐檔 39 檔與三個市場別實測差距都是 0.000000。

改成加權平均會得到不同的拆分比例（實測相差 382,020：FIFO 3,987,852 已實現 vs
加權平均 3,605,832），但總損益與這個等式都不受影響。

驗證用的手算案例（能區分兩種算法）：買 100 股 @100、買 100 股 @200、
賣 100 股 @250，現價 250 → FIFO 應為已實現 +15,000／未實現 +5,000；
加權平均會是 +10,000／+10,000。

幣別：個股卡片用原幣（`t.amount`），台幣彙總用交易當日匯率換算的 `t.twd`，
市值則用即時匯率 —— 與 `pl` 的處理方式一致。
...
> **這是 2026/09/02 匯入後、AMSC 改列美股當下的快照，不是恆定值。**
> 每新增或刪除一筆交易它就會變（例如當天稍後又下了幾筆單，台股就變成
> 6,134,066、合計 21,222,830）。
>
> 它的用途是「改計算邏輯**前後**拿同一份資料比對」，不是「永遠應該等於這個值」。
> 看到對不上時先確認交易筆數有沒有變，不要急著去修一個不存在的 bug。
>
> AMSC 的 `market` 從「台股」改成「美股」後，那筆 +48,920 的已實現獲利換了桶子，
> 所以這兩格與最初匯入時（5,941,515 ／ 15,150,371）不同，合計不受影響。
...
**冷啟動已壓到 1 次往返**（`klfan_bootstrap()`，見 `005_bootstrap.sql`）。
字典編碼已驗證無損：把輸出照 `applyBootstrapTx()` 的方式解碼回去重算，
與直接從資料表計算的結果完全相同（2026/09/02，1,484 筆）。
標的、報價、全部交易、字典一次回完，93KB。實測（往返 3 秒）：
...
1. **不要為了「保險」把 bootstrap 拆回多次查詢。** 往返次數才是成本，資料大小
   幾乎不影響（2.7KB 要 3.4s、61KB 要 3.1s）。
2. **回應被截斷時 `t` 會少於 `n`**，`loadFromDb` 會自動退回分頁載入。
   交易量長大到單次塞不下時這條路會接手，不要把它拿掉。
3. **`refresh_klfan_quotes()` 會把新報價一起回傳**，所以更新後不必再讀一次
   `klfan_quotes`。
